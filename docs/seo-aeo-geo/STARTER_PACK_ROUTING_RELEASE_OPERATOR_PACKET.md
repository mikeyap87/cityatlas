# CityAtlas Starter-Pack Routing Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the starter-pack routing CityAtlas release after the nine hosted source-backed queues are already resolved.

This packet does not replace:

- `STARTER_PACK_ROUTING_RELEASE_QUEUE_PACKET.md`
- `STARTER_PACK_ROUTING_RELEASE_GO_NO_GO_CHECKLIST.md`
- `STARTER_PACK_ROUTING_RELEASE_SLICE_HANDOFF.md`
- `STARTER_PACK_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: starter-pack routing release only

## Intended Change

Promote this route after the nine hosted source-backed queues are live:
...
- Local route proof confirms the guide hero, direct-path section, FAQ block, related guides, and route-specific title, description, robots, canonical, and breadcrumb/article JSON-LD behavior on the local base URL.
...
- the starter-pack routing release is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the ninth hosted source-backed queue

## Exact Release Lane Build Order

1. Refresh the live base after the nine hosted source-backed queues are already live.
2. Cut a fresh release lane from that live base.
3. Re-cut only the intended payload.
   - Use `STARTER_PACK_ROUTING_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `STARTER_PACK_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-ninth-source-backed-queue> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-ninth-source-backed-queue>` exists locally and the release work is moved into a clean branch with actual commit history
...
```bash
curl -I http://127.0.0.1:5178/vancouver/guides/vancouver-itinerary-starter-pack-which-cityatlas-page-should-you-open-first
```
...
- route-specific title, description, robots, canonical, and breadcrumb/article JSON-LD still match expectations
...
- exact deployed state that already includes the nine hosted source-backed queues
...
- the real live git base after the nine hosted source-backed queues is not available in this workspace checkout
