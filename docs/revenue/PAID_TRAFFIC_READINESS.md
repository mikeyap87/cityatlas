...
As of July 2, 2026, the fresh live-based release lane builds cleanly, the local desktop/mobile browser pass is green, and the local paid-traffic verifier is green. The local request path now also adds campaign/referral context to the copied or email-ready business request, and the verifier proved the copied request includes the paid-test UTM context.

That same fresh local CRO pass also rechecked the public business funnel itself: the pricing page keeps the first request-first move clear, the partner-preview page explains what the first response should include, and the business-request form gives guided starting prompts instead of leaving the operator with a blank box.
...
- Local CRO attribution and route-memory hardening: prepared and locally proven on a fresh clean release lane, but not deployed from this lane yet. It should go through an approved release before the live site depends on it.
...
- A fresh local browser pass on 2026-07-02 confirmed the planner and saved-plan route layer on desktop and mobile now gives route timing, travel-mode switching, inline Google Maps embed proof in the local env, Google Maps handoff links, share text, reorderable saved-plan rows, explicit route feedback, and visited/skipped progress states.
- A fresh local CRO pass on 2026-07-02 also rechecked pricing, partner-preview, and submit behavior in desktop/mobile browser review after the clearer first-response and guided-request improvements.
- A fresh local paid-traffic proof on 2026-07-02 passed the request-first funnel through `business_request_saved_for_later` with `measurementReady: true`, `localBusinessFunnelPassed: true`, and `copiedRequestHasCampaignContext: true`.
...
- The latest campaign-context improvement and newest route-memory layer are proven locally but not deployed from this lane.
...
- This clean release lane is local proof only until an approved deploy and hosted smoke are completed.
...
4. Prefer deploying the local campaign-context plus route-memory hardening through this fresh clean release lane before spend starts.
...
Use this fresh clean release lane to carry the local campaign-context plus route-memory hardening through an approved deploy, rerun hosted proof, then start only a tiny request-first paid test. Keep the real City Partner checkout proof as a separate live-money approval step.
