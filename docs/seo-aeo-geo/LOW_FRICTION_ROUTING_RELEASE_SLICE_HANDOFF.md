# CityAtlas Low-Friction Routing Release Slice Handoff

## Purpose

Use this packet when the low-friction routing release is being re-cut from live truth after the earlier source-backed queues and the starter-pack routing release are already resolved.

This file answers a narrower question than the routing queue packet:

`Which CityAtlas files and surfaces belong to the low-friction routing release slice, and which nearby work should stay out?`

Use `docs/seo-aeo-geo/LOW_FRICTION_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe transplant rules when this slice is re-cut from live truth.
...
- `src/data/seed.ts`
  - contains the low-friction guide slug
  - contains the low-friction guide copy, FAQ content, and proof note
  - contains the `resourceLinks` list that powers the direct-path routing section
  - this is a shared editorial data file, so use the transplant checklist to keep only the low-friction guide record in this release slice
...
- `src/features/public/GuideDetailPage.tsx`
  ...
  - contains the generic guide route output that shows the low-friction guide’s internal routing cards
...
- `public/sitemap.xml`
  - should include the low-friction guide route
  - should not widen the release into other later guide routes

- `public/llms.txt`
  - should list the low-friction guide as local-ready-to-live when appropriate
...
- `src/features/public/HomePage.tsx`
  - currently links the low-friction guide inside a broader route-intent block
  - do not assume the whole block belongs to this release slice

- `src/features/public/CityPage.tsx`
  - currently links the low-friction guide together with roundup, starter-pack, and standards links
  - do not transplant the whole button block into the low-friction release lane

- `src/features/public/AboutPage.tsx`
  - currently does not need to widen into this release just because the guide exists
  - do not add this surface unless the exact link can be isolated cleanly on the release lane
...
- `src/components/Seo.tsx`
  ...
  - current local proof confirms the low-friction guide uses this correctly against the configured local base URL
...
These docs belong to the approval and proof layer for the low-friction release. They are safe to include in a docs/proof handoff, but they are not the user-facing public payload themselves.

## Default Exclusions

Keep these out of the low-friction routing release unless a later explicit approval changes scope:

- the nine source-backed release queues
- the starter-pack routing release
- the guide-roundup routing release
...
The current local branch mixes the low-friction guide with broader shared guide rendering and mixed link-only surfaces.

Because of that, the future release lane should follow this order:

- keep the required low-friction guide record and routing-support slice
- cherry-pick the mixed shared link surfaces only if the exact low-friction links can be isolated cleanly
...
Use `docs/seo-aeo-geo/LOW_FRICTION_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` as the exact rule set.

## Local Proof Already Backing This Slice

Current local proof already recorded in repo docs includes:

- local `HTTP/1.1 200 OK` for the low-friction guide route
- route-level DOM proof for the direct-path section, FAQ block, related guides, and query-fit content
- honest `sitemap.xml` and `llms.txt` entries for the guide route
- `npm run seo:structure:proof` now includes the low-friction guide in the checked route set
- current machine truth now stands at `37` useful pieces, `23` guides, `14` source-backed wedge collections, `70` source-backed anchors, and `14` mapped guide-to-collection links
- current queue truth that keeps this slice after the nine source-backed queues and the earlier starter-pack routing release

## Local Metadata Caveat

- The recorded metadata and DOM proof for this slice is local-only.
