# CityAtlas UBC Discovery Source-Backed Release Go/No-Go Checklist

## Purpose

Use this as the exact go/no-go handoff before any live action for the eighth queued source-backed release.

This checklist does not replace `UBC_DISCOVERY_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md` or `UBC_DISCOVERY_SOURCE_BACKED_RELEASE_OPERATOR_PACKET.md`. It turns that packet into an approval-ready live-release gate while the operator packet stays the exact execution runbook.

Use `docs/seo-aeo-geo/UBC_DISCOVERY_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the exact path-scoped file map that belongs in the UBC discovery queued release slice.
Use `docs/seo-aeo-geo/UBC_DISCOVERY_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe rules that keep mixed shared files from widening the release.
...
Promote the eighth queued local-ready source-backed release to the live public domain:
...
- internal links from the homepage, city page, guide library, and other crawlable public surfaces only where the UBC discovery links can be isolated cleanly
...
Treat the intended live payload as the narrow UBC discovery slice only:

- UBC discovery source-backed page
- matching UBC discovery guide
...
- Kitsilano scenic, west-side daytime, False Creek culture, or later source-backed release work
...
- UBC discovery batch passed `npm run typecheck`
- UBC discovery batch passed `npm run build`
- UBC discovery batch passed `npm run readiness`
- UBC discovery page and guide both returned local `HTTP/1.1 200 OK`
- local `sitemap.xml` and `llms.txt` include the queued routes honestly
- current local browser-head proof now covers the two UBC discovery routes and confirms route-specific title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both routes
- `npm run seo:proof` passes for the current content machine with zero failures
- `npm run seo:structure:proof` passes for the current shared route-structure layer with zero failures and includes the UBC discovery source-backed page plus matching guide in the checked route set
...
- repo truth confirms the UBC discovery guide maps to `vancouver_ubc_discovery_starters`
- repo truth also confirms `src/features/public/TrustPages.tsx` keeps the UBC discovery source-backed page CTA pointed at the matching guide and `src/features/public/GuideDetailPage.tsx` contains the UBC discovery source-backed guide section
...
- the recorded UBC discovery metadata proof is local-only, including direct browser-head proof on the two UBC discovery routes
...
- hosted rendering of the two UBC discovery routes on `city.univenturestudio.com`
...
3. The intended payload stays limited to the UBC discovery slice.
4. The release lane follows `docs/seo-aeo-geo/UBC_DISCOVERY_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for every shared file and defers mixed link-only surfaces if they would also ship later wedges.
...
- the exact UBC discovery release lane does not receive fresh local browser-head proof before release
...
3. deploy only the intended UBC discovery payload
...
If hosted smoke fails, roll back to deployment `dpl_8t2Z6dZoRNDL8e8fGw4WDwrk6op3` or the last known good equivalent and keep the UBC discovery queue local-only until the issue is fixed.
