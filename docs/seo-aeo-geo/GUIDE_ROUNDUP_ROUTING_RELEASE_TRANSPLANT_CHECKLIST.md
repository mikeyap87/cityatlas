# CityAtlas Guide-Roundup Routing Release Transplant Checklist

## Purpose

Use this when the guide-roundup routing release is being re-cut from live truth after the earlier source-backed queues and routing-only releases are already resolved.
...
Only this route belongs to the guide-roundup routing release:
...
Treat these as required guide-roundup files, but copy only the route-specific portions that belong to the guide above:

- `src/data/seed.ts`
  - keep the guide-roundup content
  - keep the guide-roundup FAQ content
  - keep the guide-roundup `resourceLinks` list
...
- `src/styles/components.css`
  - keep only the guide-roundup direct-path styling support that is required for `.guide-query-grid`

- `src/styles/responsive.css`
  - keep only the responsive guide-roundup direct-path styling support already proven locally

- `public/sitemap.xml`
  - add only the guide-roundup route
  - do not widen the release into other later guide routes

- `public/llms.txt`
  - add only the guide-roundup wording that matches what is actually live after release
...
These files currently bundle the guide-roundup page together with broader guide, wedge, or standards links:
...
- cherry-pick only the guide-roundup link additions if they can be isolated cleanly on the release lane
...
Treat these as shared dependencies, not guide-roundup content by default:
...
- confirm the guide-roundup route still returns `200`
- confirm the guide-roundup page still shows the expected direct-path section and local metadata behavior
- keep the later machine truth honest if the local package has advanced beyond the original guide-roundup batch counts
...
- required route and crawl slice: keep the guide-roundup page only
