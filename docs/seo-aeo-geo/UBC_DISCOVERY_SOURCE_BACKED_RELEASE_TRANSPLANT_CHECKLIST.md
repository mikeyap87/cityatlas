# UBC Discovery Source-Backed Release Transplant Checklist

## Purpose

Use this when the UBC discovery queued source-backed release is being re-cut from live truth after the earlier seven source-backed queues are already resolved.

This file keeps the release copy-safe and prevents accidental pull-in of unrelated queue or routing work.

## Keep

- the `vancouver_ubc_discovery_starters` collection id and metadata
- the `/vancouver/ubc-discovery-starters` route
- the matching guide route
- the five UBC discovery source-backed anchors only
...
- route reachability for both UBC discovery routes
- local browser-head proof on both UBC discovery routes
