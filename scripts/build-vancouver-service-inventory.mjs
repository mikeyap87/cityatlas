...
const OUTPUT_CSV = path.resolve(
  __dirname,
  "../output/growth/vancouver-current-year-service-inventory.csv",
);
const OUTPUT_SUMMARY = path.resolve(
  __dirname,
  "../output/growth/vancouver-current-year-service-inventory-summary.json",
);
const OUTPUT_OPERATOR_JSON = path.resolve(
  __dirname,
  "../public/operator/vancouverOfficialServiceInventory.json",
);
const OUTPUT_OPERATOR_SUMMARY_JSON = path.resolve(
  __dirname,
  "../public/operator/vancouverOfficialServiceInventorySummary.json",
);
const PAGE_SIZE = 100;
const SERVICE_TYPES = [
  "Beauty Services",
  ...
  "Vehicle Repair Detailing and Washing Services",
];
const RUN_DATE = "2026-06-22";
const USE_EXISTING_CSV = process.argv.includes("--from-existing-csv");

function buildWhereClause() {
  return `status="Issued" and folderyear="26" and businesstype in ("${SERVICE_TYPES.join('","')}")`;
}
...
async function fetchJson(url) {
  const response = await fetch(url, {
    headers: {
      ...
      "user-agent": "CityAtlas local service inventory builder",
    },
  });
...
function mapCategoryPrimary(businesstype) {
  if (businesstype === "Beauty Services") return "beauty";
  if (businesstype === "Building Repair and Maintenance") return "home-services";
  if (businesstype === "Sport and Fitness Instruction") return "fitness";
  if (businesstype === "Vehicle Repair Detailing and Washing Services") return "automotive";
...
function toInventoryRow(record) {
  ...
  return {
    ...
    sourceScope: "vancouver_services_issued_2026",
    ...
    outreachPriority: "",
    notes:
      "Imported from the official City of Vancouver business licences dataset. Website, public contact path, and outreach fit still need official-site verification.",
