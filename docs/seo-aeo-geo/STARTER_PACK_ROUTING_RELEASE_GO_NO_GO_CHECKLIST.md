# CityAtlas Starter-Pack Routing Release Go/No-Go Checklist

## Purpose

Use this as the exact go/no-go handoff before any live action for the later starter-pack routing release.

This checklist does not replace `STARTER_PACK_ROUTING_RELEASE_QUEUE_PACKET.md` or `STARTER_PACK_ROUTING_RELEASE_OPERATOR_PACKET.md`. It turns that packet into an approval-ready live-release gate while the operator packet stays the exact execution runbook.

Use `docs/seo-aeo-geo/STARTER_PACK_ROUTING_RELEASE_SLICE_HANDOFF.md` for the exact path-scoped file map that belongs in the starter-pack routing release slice.
Use `docs/seo-aeo-geo/STARTER_PACK_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe rules that keep mixed shared files from widening the release.
...
Promote the later starter-pack routing release to the live public domain:
...
- internal links from the homepage, city page, About page, guide library, footer, and the guide itself only where those starter-pack links can be isolated cleanly
...
Treat the intended live payload as the narrow starter-pack routing slice only:

- the starter-pack guide route
...
- the low-friction or guide-roundup routing releases
...
- starter-pack batch passed `npm run typecheck`
- starter-pack batch passed `npm run build`
- starter-pack batch passed `npm run readiness`
- starter-pack guide returned local `HTTP/1.1 200 OK`
- local `sitemap.xml` and `llms.txt` include the queued route honestly
- current local browser proof covers the route hero, direct-path section, FAQ block, related guides, and route-specific title, description, robots, canonical, and breadcrumb/article JSON-LD behavior
- `npm run seo:proof` passes for the current content machine with zero failures
- `npm run seo:structure:proof` passes for the current shared route-structure layer with zero failures and includes the starter-pack routing guide in the checked route set
...
- the route remains intentionally queued behind the nine hosted source-backed releases
...
- hosted rendering of the starter-pack guide on `city.univenturestudio.com`
...
3. The intended payload stays limited to the starter-pack routing slice.
4. The release lane follows `docs/seo-aeo-geo/STARTER_PACK_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for every shared file and defers mixed link-only surfaces if they would also ship unrelated later work.
...
- the exact starter-pack release lane does not receive fresh local route proof before release
...
3. deploy only the intended starter-pack routing payload
...
If hosted smoke fails, roll back to deployment `dpl_8t2Z6dZoRNDL8e8fGw4WDwrk6op3` or the last known good equivalent and keep the starter-pack route local-only until the issue is fixed.
