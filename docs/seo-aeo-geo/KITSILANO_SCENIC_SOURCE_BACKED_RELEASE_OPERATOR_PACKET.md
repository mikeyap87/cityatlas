# CityAtlas Kitsilano Scenic Source-Backed Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the Kitsilano scenic hosted source-backed CityAtlas release after the first four hosted queues are already resolved and before the west-side daytime queue.

This packet does not replace:

- `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md`
- `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_GO_NO_GO_CHECKLIST.md`
- `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md`
- `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: fifth hosted source-backed queue only

## Intended Change

Promote these two routes together after the first four queues are live:
...
- Local browser-head proof confirms title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both Kitsilano scenic routes.
- Local route checks for the Kitsilano scenic page and Kitsilano scenic guide both return `HTTP/1.1 200 OK` on `http://127.0.0.1:5178`.
...
- the Kitsilano scenic hosted queue is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the first four hosted queues

## Exact Release Lane Build Order

1. Refresh the live base after the first four queues are already live.
   - Identify the real live branch, ref, or commit that matches the public CityAtlas surface after the returning-visitor queue.
   - Do not ship from the current mixed workspace branch.
2. Cut a fresh release lane from that live base.
   - Name it specifically for the Kitsilano scenic queue.
   - Keep the lane limited to the Kitsilano scenic slice.
3. Re-cut only the intended payload.
   - Use `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `KITSILANO_SCENIC_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-returning-visitor-queue> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-returning-visitor-queue>` exists locally and the release work is moved into a clean branch with actual commit history
...
```bash
curl -I http://127.0.0.1:5178/vancouver/kitsilano-scenic-starters
curl -I http://127.0.0.1:5178/vancouver/guides/kitsilano-scenic-route-starter-guide-for-slower-vancouver-evenings
```
...
- the Kitsilano scenic guide section renders
...
- exact deployed state that already includes the first four hosted queues

## Hosted Smoke Sequence After Deploy

Run immediately after the live release:

1. `curl -I` the two Kitsilano scenic routes and confirm `200`.
2. Check hosted `sitemap.xml` for the Kitsilano scenic source-backed page and guide.
3. Check hosted `llms.txt` for honest route/state wording.
4. Render the Kitsilano scenic source-backed page and guide in a browser and confirm:
   - source cards
   - correction links
...
- the real live git base after the returning-visitor queue is not available in this workspace checkout
