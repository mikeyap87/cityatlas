# CityAtlas West-Side Daytime Source-Backed Release Go/No-Go Checklist

## Purpose

Use this as the exact go/no-go handoff before any live action for the sixth queued source-backed release.

This checklist does not replace `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md` or `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_OPERATOR_PACKET.md`. It turns that packet into an approval-ready live-release gate while the operator packet stays the exact execution runbook.

Use `docs/seo-aeo-geo/WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the exact path-scoped file map that belongs in the west-side daytime queued release slice.
Use `docs/seo-aeo-geo/WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe rules that keep mixed shared files from widening the release.
...
Promote the sixth queued local-ready source-backed release to the live public domain:
...
- internal links from the homepage, city page, guide library, and other crawlable public surfaces only where the west-side daytime links can be isolated cleanly
...
Treat the intended live payload as the narrow west-side daytime slice only:

- west-side daytime source-backed page
- matching west-side daytime guide
...
- Kitsilano scenic or later source-backed release work
...
- west-side daytime batch passed `npm run typecheck`
- west-side daytime batch passed `npm run build`
- west-side daytime batch passed `npm run readiness`
- west-side daytime page and guide both returned local `HTTP/1.1 200 OK`
- local `sitemap.xml` and `llms.txt` include the queued routes honestly
- current local browser-head proof now covers the two west-side daytime routes and confirms route-specific title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both routes
- `npm run seo:proof` passes for the current content machine with zero failures
- `npm run seo:structure:proof` passes for the current shared route-structure layer across `33` key routes with zero failures and includes the west-side daytime source-backed page plus matching guide in the checked route set
- `npm run seo:docs:proof` passes for the approval-doc layer with zero failures
- `npm run seo:smoke:westside` passes on the local preview base and rehearses the two west-side daytime routes, route-level title/robots/canonical/JSON-LD behavior, pairing fragments, `sitemap.xml`, `llms.txt`, and the noindex surface
...
- repo truth confirms the west-side daytime guide maps to `vancouver_west_side_daytime_starters`
- repo truth also confirms `src/features/public/TrustPages.tsx` keeps the west-side daytime source-backed page CTA pointed at the matching guide and `src/features/public/GuideDetailPage.tsx` contains the west-side daytime source-backed guide section
...
- the recorded west-side daytime metadata proof is local-only, including direct browser-head proof on the two west-side daytime routes
...
- hosted rendering of the two west-side daytime routes on `city.univenturestudio.com`
...
3. The intended payload stays limited to the west-side daytime slice.
4. The release lane follows `docs/seo-aeo-geo/WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for every shared file and defers mixed link-only surfaces if they would also ship later wedges.
...
- the exact west-side daytime release lane does not receive fresh local browser-head proof before release
...
3. deploy only the intended west-side daytime payload
...
3. render the west-side daytime source-backed page and guide in a browser and confirm:
   - source cards
   - correction links
   - the west-side daytime guide section
   - the west-side daytime page-to-guide / guide-to-page pairing
...
If hosted smoke fails, roll back to deployment `dpl_8t2Z6dZoRNDL8e8fGw4WDwrk6op3` or the last known good equivalent and keep the west-side daytime queue local-only until the issue is fixed.
