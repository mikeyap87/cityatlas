# CityAtlas West-Side Daytime Source-Backed Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the west-side daytime hosted source-backed CityAtlas release after the first five hosted queues are already resolved and before the False Creek culture queue.

This packet does not replace:

- `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md`
- `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_GO_NO_GO_CHECKLIST.md`
- `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md`
- `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: sixth hosted source-backed queue only

## Intended Change

Promote these two routes together after the first five queues are live:
...
- `npm run seo:structure:proof` passes across `33` key routes, including every currently queued source-backed guide/page pair, the later routing guides, and the protected noindex routes.
...
- Local browser-head proof confirms title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both west-side daytime routes.
- Local route checks for the west-side daytime page and west-side daytime guide both return `HTTP/1.1 200 OK` on `http://127.0.0.1:5178`.
...
- the west-side daytime hosted queue is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the first five hosted queues

## Exact Release Lane Build Order

1. Refresh the live base after the first five queues are already live.
   - Identify the real live branch, ref, or commit that matches the public CityAtlas surface after the Kitsilano scenic queue.
   - Do not ship from the current mixed workspace branch.
2. Cut a fresh release lane from that live base.
   - Name it specifically for the west-side daytime queue.
   - Keep the lane limited to the west-side daytime slice.
3. Re-cut only the intended payload.
   - Use `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `WEST_SIDE_DAYTIME_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-kitsilano-queue> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-kitsilano-queue>` exists locally and the release work is moved into a clean branch with actual commit history
...
Then verify local route `200` checks if you need a direct route-status spot check outside the smoke artifact:

```bash
curl -I http://127.0.0.1:5178/vancouver/west-side-daytime-starters
curl -I http://127.0.0.1:5178/vancouver/guides/where-should-you-start-a-west-side-vancouver-daytime-plan
```
...
- the west-side daytime guide section renders
...
- exact deployed state that already includes the first five hosted queues

## Hosted Smoke Sequence After Deploy

Run immediately after the live release:

1. `curl -I` the two west-side daytime routes and confirm `200`.
2. Check hosted `sitemap.xml` for the west-side daytime source-backed page and guide.
3. Check hosted `llms.txt` for honest route/state wording.
4. Render the west-side daytime source-backed page and guide in a browser and confirm:
   - source cards
   - correction links
...
- the real live git base after the Kitsilano scenic queue is not available in this workspace checkout
