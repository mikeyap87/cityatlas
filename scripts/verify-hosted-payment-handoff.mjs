...
async function main() {
  ...
  try {
    ...
    const reviewBeforePayment = pageTextIncludes(pageText, /Review before payment/i);
    const startRequestBeforePayment = pageTextIncludes(
      pageText,
      /Start the request before you decide on payment|Start with the request/i,
    );
    const requestFirst = pageTextIncludes(pageText, /Request first|request-first|No payment is required to start/i);
    const noInstantPublication = pageTextIncludes(
      pageText,
      /Paid checkout does not turn on instant publication|No public profile until facts are checked/i,
    );
