...
export interface MissionScheduleEntry {
  stepIndex: number;
  startMinutes: number;
  endMinutes: number;
  startLabel: string;
...
export function parseMissionClockLabel(value?: string) {
...
export function formatMissionClockLabel(totalMinutes?: number) {
...
export function getMissionSelectedStartTime(mission: CityMission, plan: MissionPlanState) {
  return plan.selectedStartTime ?? mission.startOptions?.[0];
}

export function getMissionSelectedStartMinutes(mission: CityMission, plan: MissionPlanState) {
  return parseMissionClockLabel(getMissionSelectedStartTime(mission, plan));
}

export function getMissionSchedule(
  mission: CityMission,
  plan: MissionPlanState,
): MissionScheduleEntry[] {
  const selectedStartMinutes = getMissionSelectedStartMinutes(mission, plan);
...
export function getMissionEndTimeLabel(
  mission: CityMission,
  plan: MissionPlanState,
) {
  return getMissionSchedule(mission, plan).at(-1)?.endLabel;
}
...
export function getMissionShareText(
  ...
  const scheduleByIndex = new Map(
    getMissionSchedule(mission, plan).map((entry) => [entry.stepIndex, entry]),
  );

  const lines = [
    `${mission.title} via CityAtlas`,
    selectedStartTime && endTime
      ? `Suggested timing: ${selectedStartTime} start - ${endTime} finish`
