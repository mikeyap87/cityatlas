...
- Google Maps route links are intentionally API-key-free and open Google Maps directions with origin, destination, waypoints, and walking mode. Embedded on-page Google route previews are optional and gated behind `VITE_GOOGLE_MAPS_EMBED_API_KEY`; without the key the public UI falls back to the visible stop-order panel. The code and verifier are ready for Maps Embed API URLs. Google account 2-step verification is now complete and a clean Google Cloud `CityAtlas` project exists at project id `cityatlas-501120`, but Maps Embed API enablement is currently blocked by Google's card/free-trial verification step. No restricted production embed key has been created, copied, installed, or deployed yet.
...
12. For embedded route previews, finish the Google Cloud card/free-trial verification for the `CityAtlas` project (`cityatlas-501120`) only if the owner approves that account step, create a Maps Embed API key restricted to the CityAtlas domains and the Maps Embed API, wire `VITE_GOOGLE_MAPS_EMBED_API_KEY` only after restriction, then rerun local and hosted route-map proof. Do not claim embedded on-page Google maps are live until this passes.
