...
- Google Maps route links are intentionally API-key-free and open Google Maps directions with origin, destination, waypoints, and the visitor-selected travel mode. Route pages now also include plan-length controls, travel-mode controls, mini itineraries, no-login route progress, skipped-stop alternatives, shareable route links with a manual copy fallback, and route-specific business review CTAs. Embedded on-page Google route previews are optional and gated behind `VITE_GOOGLE_MAPS_EMBED_API_KEY`; without the key the public UI keeps the visible stop-order/checkoff panel. The owner completed Google setup and provided a browser Maps key outside committed source; this release lane uses it only in ignored local `.env.local` for proof. A restricted production embed key has not been installed in Vercel or hosted-smoked yet, and the pasted key should be treated as exposed unless referrer and API restrictions are confirmed.
