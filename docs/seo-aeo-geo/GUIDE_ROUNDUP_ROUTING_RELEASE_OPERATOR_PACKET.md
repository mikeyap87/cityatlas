# CityAtlas Guide-Roundup Routing Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the guide-roundup routing CityAtlas release after the low-friction routing release is already resolved.

This packet does not replace:

- `GUIDE_ROUNDUP_ROUTING_RELEASE_QUEUE_PACKET.md`
- `GUIDE_ROUNDUP_ROUTING_RELEASE_GO_NO_GO_CHECKLIST.md`
- `GUIDE_ROUNDUP_ROUTING_RELEASE_SLICE_HANDOFF.md`
- `GUIDE_ROUNDUP_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: guide-roundup routing release only

## Intended Change

Promote this route after the nine hosted source-backed queues plus the starter-pack and low-friction routing releases are live:
...
- Local route proof confirms the routing sections and shared guide-detail output on the local base URL.
...
- the guide-roundup routing release is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the low-friction routing release

## Exact Release Lane Build Order

1. Refresh the live base after the low-friction routing release is already live.
2. Cut a fresh release lane from that live base.
3. Re-cut only the intended payload.
   - Use `GUIDE_ROUNDUP_ROUTING_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `GUIDE_ROUNDUP_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-low-friction-routing-release> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-low-friction-routing-release>` exists locally and the release work is moved into a clean branch with actual commit history
...
```bash
curl -I http://127.0.0.1:5178/vancouver/guides/cityatlas-guide-roundup-which-vancouver-route-should-you-open-by-situation
```

Then verify local route behavior on the same guide:

- routing sections render
- expected internal links remain intact
...
- exact deployed state that already includes the earlier source-backed queues plus the starter-pack and low-friction routing releases
...
- the real live git base after the low-friction routing release is not available in this workspace checkout
