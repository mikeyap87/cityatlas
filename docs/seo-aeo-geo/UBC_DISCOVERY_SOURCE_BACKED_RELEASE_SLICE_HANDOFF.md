# UBC Discovery Source-Backed Release Slice Handoff

## Purpose

This handoff isolates the eighth hosted source-backed release slice for the UBC discovery wedge.
...
Use `docs/seo-aeo-geo/UBC_DISCOVERY_SOURCE_BACKED_RELEASE_GO_NO_GO_CHECKLIST.md` as the exact live gate for this slice before any deploy approval.
...
- `npm run seo:structure:proof` now passes and includes the UBC discovery source-backed page plus matching guide in the checked route set
...
- local browser-head proof now covers the two UBC discovery routes and confirms title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing
