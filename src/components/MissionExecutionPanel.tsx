...
export function MissionExecutionPanel({
  ...
  const orderedStepIndices = useMemo(() => {
    const indices = mission.steps.map((_, index) => index);
    if (nextStepIndex <= 0) return indices;
    return [...indices.slice(nextStepIndex), ...indices.slice(0, nextStepIndex)];
  }, [mission.steps, nextStepIndex]);
      ...
      <div className="mission-planner-shell">
        ...
        <div className="mission-planner-timeline" id={routeTimelineId}>
          <div className="mission-planner-timeline-header">
            <div>
              <p className="section-label">Stop deck</p>
              <h3>Follow the route one stop at a time</h3>
              <p>
                {nextStepTarget
                  ? `Start with ${nextStepTarget.label}. The next few stops stay tucked underneath until you need them.`
          ...
          <ol className="route-timeline route-timeline-detailed route-timeline-rail route-timeline-deck">
            {orderedStepIndices.map((stepIndex, orderedIndex) => {
              ...
              const schedule = scheduleByIndex[stepIndex] as { rangeLabel?: string } | undefined;
                  ...
                  >
                    <summary className="route-step-summary">
                      ...
                      <span className="route-step-disclosure-tag" aria-hidden="true">
                        {isExpanded ? "Hide card" : "Open card"} <ChevronDownIcon />
                      </span>
                    </summary>
