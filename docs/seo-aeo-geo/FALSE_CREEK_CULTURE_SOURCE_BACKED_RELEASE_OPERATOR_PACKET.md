# CityAtlas False Creek Culture Source-Backed Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the False Creek culture hosted source-backed CityAtlas release after the first six hosted queues are already resolved and before the UBC discovery queue.

This packet does not replace:

- `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md`
- `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_GO_NO_GO_CHECKLIST.md`
- `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md`
- `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: seventh hosted source-backed queue only

## Intended Change

Promote these two routes together after the first six queues are live:
...
- Local browser-head proof confirms title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both False Creek culture routes.
- Local route checks for the False Creek culture page and False Creek culture guide both return `HTTP/1.1 200 OK` on `http://127.0.0.1:5178`.
...
- the False Creek culture hosted queue is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the first six hosted queues

## Exact Release Lane Build Order

1. Refresh the live base after the first six queues are already live.
   - Identify the real live branch, ref, or commit that matches the public CityAtlas surface after the west-side daytime queue.
   - Do not ship from the current mixed workspace branch.
2. Cut a fresh release lane from that live base.
   - Name it specifically for the False Creek culture queue.
   - Keep the lane limited to the False Creek culture slice.
3. Re-cut only the intended payload.
   - Use `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-west-side-daytime-queue> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-west-side-daytime-queue>` exists locally and the release work is moved into a clean branch with actual commit history
...
```bash
curl -I http://127.0.0.1:5178/vancouver/false-creek-culture-starters
curl -I http://127.0.0.1:5178/vancouver/guides/where-should-you-start-a-false-creek-vancouver-culture-afternoon
```
...
- the False Creek culture guide section renders
...
- exact deployed state that already includes the first six hosted queues
...
2. Check hosted `sitemap.xml` for the False Creek culture source-backed page and guide.
...
- the real live git base after the west-side daytime queue is not available in this workspace checkout
