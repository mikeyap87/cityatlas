# CityAtlas Garden Day Source-Backed Release Operator Packet

## Purpose

Use this as the exact local operator runbook when the owner approves the garden-day hosted source-backed CityAtlas release after the first eight hosted queues are already resolved.

This packet does not replace:

- `GARDEN_DAY_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md`
- `GARDEN_DAY_SOURCE_BACKED_RELEASE_GO_NO_GO_CHECKLIST.md`
- `GARDEN_DAY_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md`
- `GARDEN_DAY_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md`
...
- Release scope: ninth hosted source-backed queue only

## Intended Change

Promote these two routes together after the first eight queues are live:
...
- Local browser-head proof confirms title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both garden-day routes.
- Local route checks for the garden-day page and garden-day guide both return `HTTP/1.1 200 OK` on `http://127.0.0.1:5178`.
...
- the garden-day hosted queue is approval-ready in docs and local proof only
- the live action still requires a real git checkout or branch that contains the actual CityAtlas live base after the first eight hosted queues

## Exact Release Lane Build Order

1. Refresh the live base after the first eight queues are already live.
   - Identify the real live branch, ref, or commit that matches the public CityAtlas surface after the UBC discovery queue.
   - Do not ship from the current mixed workspace branch.
2. Cut a fresh release lane from that live base.
   - Name it specifically for the garden-day queue.
   - Keep the lane limited to the garden-day slice.
3. Re-cut only the intended payload.
   - Use `GARDEN_DAY_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the file map.
   - Use `GARDEN_DAY_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for shared-file rules.
   - Defer mixed link-only surfaces if isolating them would also ship later routing releases.
...
node /Users/michaelyap/Documents/Codex/Workspace/univenture/scripts/release-safety-check.mjs \
  ...
  --base <live-base-ref-after-ubc-discovery-queue> \
...
- from this workspace state, the same check is blocked until a real `<live-base-ref-after-ubc-discovery-queue>` exists locally and the release work is moved into a clean branch with actual commit history
...
```bash
curl -I http://127.0.0.1:5178/vancouver/garden-day-starters
curl -I http://127.0.0.1:5178/vancouver/guides/where-should-you-start-a-vancouver-garden-and-conservatory-day
```
...
- the garden-day guide section renders
...
- exact deployed state that already includes the first eight hosted queues
...
2. Check hosted `sitemap.xml` for the garden-day source-backed page and guide.
...
- the real live git base after the UBC discovery queue is not available in this workspace checkout
