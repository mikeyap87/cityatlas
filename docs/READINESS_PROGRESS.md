...
Generated: 2026-07-01T21:07:14.262Z
...
| Public product | 98% | Public app, City Missions, business flow, draft terms/privacy, crawl assets, live crawlable Univenture subdomain, HTTPS, robots.txt, route-level noindex guards, hosted owner-route guard, CityAtlas logo, favicon, and brand guide. | Real source-approved inventory, stronger social preview image, and more source-backed destination pages. | Expand source-backed guides and destination pages now that the approved domain is crawlable. |
...
| Revenue system | 92% | Package framing, draft terms/privacy, Stripe product manifest, accepted Stripe Billing/Checkout plan, live CAD pricing, confirmed live CityAtlas product and recurring price IDs, live Stripe-hosted payment links for both paid packages, local live-payment env wiring, env-gated payment-link handoff support on the public business funnel, local payment-handoff verification, hosted payment-handoff verification, public pricing-page checkout CTA cutover on the approved domain, local request-copy campaign-attribution proof, route-map duration and Google Maps handoff proof, plan-length and travel-mode controls, mini itineraries, shareable route-link fallback, alternate-stop links, route-specific business review CTA, Date Night revenue proof loop, and reply-summary command. | Final refund policy, one real checkout proof, clean hosted proof after the latest local attribution and route-map hardening is released, restricted production Maps Embed API key confirmation plus hosted iframe proof, and first real paid-click quality review. | Keep the tiny paid test request-first, prove one real City Partner checkout separately before self-serve charging claims, and rerun hosted proof after any approved release. |
...
- embedded Google route previews until a restricted production Maps Embed API key is confirmed and hosted route-map proof passes
...
Use the current clean CRO release lane to carry the local attribution and route-planner hardening through approved deploy proof, keep the first paid test tiny and request-first, and prove one real City Partner checkout separately before self-serve charging claims.
