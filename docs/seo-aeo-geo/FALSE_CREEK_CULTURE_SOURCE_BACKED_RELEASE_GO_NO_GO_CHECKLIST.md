# CityAtlas False Creek Culture Source-Backed Release Go/No-Go Checklist

## Purpose

Use this as the exact go/no-go handoff before any live action for the seventh queued source-backed release.

This checklist does not replace `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_QUEUE_PACKET.md` or `FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_OPERATOR_PACKET.md`. It turns that packet into an approval-ready live-release gate while the operator packet stays the exact execution runbook.

Use `docs/seo-aeo-geo/FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_SLICE_HANDOFF.md` for the exact path-scoped file map that belongs in the False Creek culture queued release slice.
Use `docs/seo-aeo-geo/FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe rules that keep mixed shared files from widening the release.
...
Promote the seventh queued local-ready source-backed release to the live public domain:
...
- internal links from the homepage, city page, guide library, and other crawlable public surfaces only where the False Creek culture links can be isolated cleanly
...
Treat the intended live payload as the narrow False Creek culture slice only:

- False Creek culture source-backed page
- matching False Creek culture guide
...
- Kitsilano scenic, west-side daytime, or later source-backed release work
...
- False Creek culture batch passed `npm run typecheck`
- False Creek culture batch passed `npm run build`
- False Creek culture batch passed `npm run readiness`
- False Creek culture page and guide both returned local `HTTP/1.1 200 OK`
- local `sitemap.xml` and `llms.txt` include the queued routes honestly
- current local browser-head proof now covers the two False Creek culture routes and confirms route-specific title, description, robots, canonical, JSON-LD, correction-path links, and the page-to-guide / guide-to-page pairing on both routes
- `npm run seo:proof` passes for the current content machine with zero failures
- `npm run seo:structure:proof` passes for the current shared route-structure layer with zero failures and includes the False Creek culture source-backed page plus matching guide in the checked route set
...
- repo truth also confirms `src/features/public/TrustPages.tsx` keeps the False Creek culture source-backed page CTA pointed at the matching guide and `src/features/public/GuideDetailPage.tsx` contains the False Creek culture source-backed guide section
...
- the recorded False Creek culture metadata proof is local-only, including direct browser-head proof on the two False Creek culture routes
...
- hosted rendering of the two False Creek culture routes on `city.univenturestudio.com`
...
3. The intended payload stays limited to the False Creek culture slice.
4. The release lane follows `docs/seo-aeo-geo/FALSE_CREEK_CULTURE_SOURCE_BACKED_RELEASE_TRANSPLANT_CHECKLIST.md` for every shared file and defers mixed link-only surfaces if they would also ship later wedges.
...
- the exact False Creek culture release lane does not receive fresh local browser-head proof before release
...
3. deploy only the intended False Creek culture payload
...
4. render the False Creek culture source-backed page and guide in a browser and confirm:
   - source cards
   - correction links
   - the False Creek culture guide section
...
If hosted smoke fails, roll back to deployment `dpl_8t2Z6dZoRNDL8e8fGw4WDwrk6op3` or the last known good equivalent and keep the False Creek culture queue local-only until the issue is fixed.
