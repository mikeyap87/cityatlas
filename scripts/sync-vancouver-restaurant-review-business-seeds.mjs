...
function buildShortlistMarkdown(shortlist, summary) {
  const lines = [
    ...
    "",
    "| Rank | Business | Municipality | Neighborhood | Category | Rating | Reviews | Email | Why now |",
    "| --- | --- | --- | --- | --- | --- | --- | --- | --- |",
    ...shortlist.map((candidate, index) => {
      const rating = toNumber(candidate.reviewRow.outscraperPlaceRating);
      const reviewCount = toNumber(candidate.reviewRow.outscraperPlaceReviewCount);
      return `| ${index + 1} | ${candidate.seed.businessName} | ${candidate.seed.municipality || "Vancouver"} | ${candidate.seed.neighborhood || "Unknown"} | ${candidate.seed.category} | ${Number.isFinite(rating) ? rating.toFixed(1) : "-"} | ${Number.isFinite(reviewCount) ? formatNumber(reviewCount) : "-"} | ${candidate.seed.email || "-"} | ${candidate.whyNow} |`;
    }),
    "",
  ];
  ...
  const seedRows = [
    ...emailMatches.pairs.map(({ importRow, reviewRow }) => {
      ...
      const municipality = inferMunicipalityFromValues(
        importRow.cityName,
        importRow.neighborhood,
        importRow.businessName,
        importRow.sourceUrl,
        importRow.website,
      ...
      return applySendLedgerOverride({
        ...
        marketScope: inferMarketScope({
          municipality,
          businessName: importRow.businessName,
          neighborhood: importRow.neighborhood,
          sourceUrl: importRow.sourceUrl || importRow.website,
          website: importRow.website || importRow.sourceUrl,
          email,
          contactPath,
          sourceProof: buildSeedProof({ importRow, reviewRow }, "email_candidate_review"),
          notes: buildSeedNotes(
