# CityAtlas Guide-Roundup Routing Release Go/No-Go Checklist

## Purpose

Use this as the exact go/no-go handoff before any live action for the later guide-roundup routing release.

This checklist does not replace `GUIDE_ROUNDUP_ROUTING_RELEASE_QUEUE_PACKET.md` or `GUIDE_ROUNDUP_ROUTING_RELEASE_OPERATOR_PACKET.md`. It turns that packet into an approval-ready live-release gate while the operator packet stays the exact execution runbook.

Use `docs/seo-aeo-geo/GUIDE_ROUNDUP_ROUTING_RELEASE_SLICE_HANDOFF.md` for the exact path-scoped file map that belongs in the guide-roundup routing release slice.
Use `docs/seo-aeo-geo/GUIDE_ROUNDUP_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for the copy-safe rules that keep mixed shared files from widening the release.
...
Promote the later guide-roundup routing release to the live public domain:
...
- internal links only where those guide-roundup links can be isolated cleanly
...
Treat the intended live payload as the narrow guide-roundup routing slice only:

- the guide-roundup route
...
- the starter-pack or low-friction routing releases
...
- guide-roundup batch passed `npm run typecheck`
- guide-roundup batch passed `npm run build`
- guide-roundup batch passed `npm run readiness`
- guide-roundup guide returned local `HTTP/1.1 200 OK`
- local `sitemap.xml` and `llms.txt` include the queued route honestly
- current local route output proof covers the routing sections and shared guide rendering for this guide
- `npm run seo:proof` passes for the current content machine with zero failures
- `npm run seo:structure:proof` passes for the current shared route-structure layer with zero failures and includes the guide-roundup routing guide in the checked route set
...
- the route remains intentionally queued behind the nine hosted source-backed releases plus the earlier starter-pack and low-friction routing releases
...
- hosted rendering of the guide-roundup page on `city.univenturestudio.com`
...
3. The intended payload stays limited to the guide-roundup routing slice.
4. The release lane follows `docs/seo-aeo-geo/GUIDE_ROUNDUP_ROUTING_RELEASE_TRANSPLANT_CHECKLIST.md` for every shared file and defers mixed link-only surfaces if they would also ship unrelated later work.
...
- the exact guide-roundup release lane does not receive fresh local route proof before release
...
3. deploy only the intended guide-roundup routing payload
...
If hosted smoke fails, roll back to deployment `dpl_8t2Z6dZoRNDL8e8fGw4WDwrk6op3` or the last known good equivalent and keep the guide-roundup route local-only until the issue is fixed.
